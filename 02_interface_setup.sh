...
apt-get install -y --no-install-recommends \
    ...
    xdotool plymouth plymouth-themes jq imagemagick || true
...
# Default Brand Asset Generator (Dark Minimal Square Logo)
if command -v convert &>/dev/null; then
    convert -size 256x256 xc:"#0c0d12" -fill "#00e5ff" -draw "circle 128,128 128,40" /usr/share/retro/wallpapers/retro-dragon.png 2>/dev/null || true
fi
...
if [ "$MODE" = "cyber" ]; then
    ...
    $KWRITE --file kdeglobals --group General --key AccentColor "#00e5ff" 2>/dev/null || true
    ...
    $KWRITE --file kdeglobals --group General --key AccentColor "#3daee9" 2>/dev/null || true
    qdbus org.kde.KWin /KWin reconfigure 2>/dev/null || true
